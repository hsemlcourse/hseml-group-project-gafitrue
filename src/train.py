...
def _evaluate(
    model, X_val, y_val, X_test, y_test, name: str, elapsed: float, notes: str = ""
) -> ExperimentResult:
...
def _fit_eval(
    model, X_tr, y_tr, X_val, y_val, X_te, y_te, name: str, notes: str = ""
    ...
    print(
        f"  {name:32s} f1_val={res.f1_macro_val:.4f} "
...
def run_baseline(split, results: list[ExperimentResult]) -> None:
    ...
    res, _ = _fit_eval(
        ...
        X_te,
        split.y_test,
        "LogReg (baseline, text only)",
        "только TF-IDF, без числовых фич",
    )
    results.append(res)
...
def run_models(split, results: list[ExperimentResult], quick: bool = False):
    ...
    print(f"  feature matrix: train={X_tr.shape}, val={X_val.shape}, " f"test={X_te.shape}")
...
def run_hp_search(split, X_tr, X_val, X_te, results, quick: bool = False) -> object:
    """Эксперимент 3: подбор гиперпараметров для топ-моделей."""
    print("\n[HP search] RandomizedSearchCV по LogReg и LightGBM/RF")

    cv = StratifiedKFold(n_splits=2 if quick else 3, shuffle=True, random_state=SEED)
    ...
    res = _evaluate(
        ...
        elapsed,
        notes=f"cv_best_f1={lr_search.best_score_:.4f}",
    )
    ...
    if HAS_LGBM:
        ...
        print(f"  LightGBM tuned f1_test={res.f1_macro_test:.4f} " f"({elapsed:.1f}s)")
        results.append(res)
...
def run_feature_importance(model, feature_pipeline, X_te, y_te, top_k: int = 30) -> pd.DataFrame:
...
def main(quick: bool = False, sample_size: int | None = None) -> None:
    ...
    print(f"  train={len(split.X_train)}, val={len(split.X_val)}, " f"test={len(split.X_test)}")
    ...
    print(f"\nФинальная модель: {final_name} " f"(f1_macro_test={final_row['f1_macro_test']})")
    ...
    with open(REPORTS_DIR / "final_model_report.txt", "w", encoding="utf-8") as f:
    ...
    print(f"  {REPORTS_DIR}/experiments.csv, final_model_report.txt, " f"feature_importance.csv")
...
if __name__ == "__main__":
    ...
    p.add_argument("--quick", action="store_true", help="прогон на 8к строк для отладки")
    p.add_argument(
        "--sample-size",
        type=int,
        default=None,
